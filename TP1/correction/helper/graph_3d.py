...
def contour(
    ...
    for i in range(len(X)):
        Z.append(
            np.fromiter(
                (
                    studied_function.function_definition(
...
def contour_and_gradient(
    ...
    plt.plot(
        coord_descent[1, 0], coord_descent[1, 1], "+", c="blue", markersize=15
    )
    plt.plot(
        coord_descent[-1, 0],
        coord_descent[-1, 1],
...
def display_3d_surface(
    studied_function, x_min=0.5, x_max=2.5, y_min=0.5, y_max=2.5, tick=0.1
):
    """
